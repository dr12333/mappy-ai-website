...
// Native <details> toggles instantly. The CSS pattern using
// ::details-content + interpolate-size: allow-keywords is supposed
// to handle the animation, but support is too patchy for production
// (falls back to instant). Driving the height + opacity animation
// from JS via the Web Animations API gives reliable timing in every
// modern browser. The plus-icon rotation stays CSS-driven (works
// fine via the [open] attribute selector).
(() => {
  const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)");
  const detailsList = document.querySelectorAll(".faq-list details");
  if (!detailsList.length) return;

  // Same curve and durations the (non-working) CSS attempt used —
  // ease-in-out cubic, slightly slower on open than close so opening
  // feels deliberate but closing is responsive.
  ...
  detailsList.forEach((details) => {
    ...
    // Animate everything inside <details> EXCEPT the <summary>. Each
    // FAQ card has a single <p>; animating that element directly is
    // sufficient.
    ...
    const runAnimation = (fromHeight, toHeight, duration, onFinish) => {
    ...
    summary.addEventListener("click", (event) => {
      // Honor prefers-reduced-motion: let the browser toggle the
      // card instantly without animation.
      if (reduceMotion.matches) return;
      event.preventDefault();

      if (details.open) {
        // Closing: measure current natural height, animate to 0,
        // flip the open attribute when the animation lands.
        const startHeight = body.scrollHeight;
        runAnimation(startHeight, 0, closeDuration, () => {
          details.open = false;
        });
      } else {
        // Opening: flip the attribute first so the body lays out and
        // we can measure its target height, then animate from 0.
        details.open = true;
        const targetHeight = body.scrollHeight;
        runAnimation(0, targetHeight, openDuration);
      }
    });
  });
})();
