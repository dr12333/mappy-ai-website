...
// <details> doesn't natively transition between states across all
// browsers (the modern ::details-content + interpolate-size pattern
// is only in recent builds). Drive the height + opacity animation
// from JS via WAAPI so the timing is reliable everywhere and we get
...
(() => {
  ...
  detailsList.forEach((details) => {
    ...
    // Animate everything inside <details> EXCEPT the <summary>. We
    // animate the first non-summary element directly (each card
    // only has one <p>); if there are multiple, the rest tag along
    // because they share the same flow height.
    ...
    summary.addEventListener("click", (event) => {
      // Honor reduced-motion: skip the animation, let the browser
      // toggle the card instantly.
      if (reduceMotion.matches) return;
      event.preventDefault();

      if (details.open) {
        // Closing: measure current natural height, animate to 0,
        // then flip the open attribute when the animation lands.
        const startHeight = body.scrollHeight;
        runAnimation(startHeight, 0, closeDuration, () => {
          details.open = false;
        });
      } else {
        // Opening: flip the attribute first so the body is in flow
        // and we can measure its target height, then animate from 0.
        details.open = true;
